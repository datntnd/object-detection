...
from Config import ConfigYolov5Val
import json
import uuid
...
mlflow.set_experiment(f"yolov5-val-{settings.pipeline_id}")
...
minioClient.fput_object(pipeline_bucket, f"{settings.user_id}/{settings.project_id}/{settings.dataset_version_id}/{settings.pipeline_id}/val.json", "val.json")
...
def run(data,
        ...
        project="yolov5/runs/val",  # save to project/name
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    output["model_result"]["Precision"] = mp
    output["model_result"]["Recall"] = mr
    output["model_result"]["mAP0.5"] = map50
    output["model_result"]["mAP0.5-0.95"] = map
    output["status"] = "Done"
    with open('val.json', 'w', encoding='utf-8') as f:
        json.dump(output, f, ensure_ascii=False, indent=4)
...
if __name__ == "__main__":
    config = {}
    opt = ConfigYolov5Val(config_dict=config)
    main(opt)
